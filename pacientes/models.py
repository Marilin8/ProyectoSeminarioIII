...
class CambioEstudioProgramado(models.Model):
    ...
    modalidad = models.CharField(max_length=20, choices=TipoEstudio.MODALIDAD_CHOICES)
    duracion_minutos = models.PositiveIntegerField()
    ...
    def resumen_diferencias(self):
        ...
        if estudio.modalidad != self.modalidad:
            nombres_modalidad = dict(TipoEstudio.MODALIDAD_CHOICES)
            diferencias.append(
                f'Modalidad: {nombres_modalidad.get(estudio.modalidad, estudio.modalidad)} → '
                f'{nombres_modalidad.get(self.modalidad, self.modalidad)}'
            )
